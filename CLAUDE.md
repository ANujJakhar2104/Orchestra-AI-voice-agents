...
**OrchestraAI** is a production-ready, real-time voice conversational assistant built using the Pipecat framework (v0.0.98). It combines STT, LLM, TTS, RAG, emotion detection, and dynamic visual UI generation (A2UI) into a streaming voice pipeline with a 1-1.5 second end-to-end latency target.

It supports **6 distinct AI agent personas** — Aanya (General Assistant), Arjun (Problem Solver), Priya (Hinglish All-Rounder), Serena (Business Strategist), Rohan (Tech Expert), and Zara (Lifestyle Coach) — each with their own voice, personality, and domain expertise. Agents can transfer mid-call with full context handoff.
...
The system prompt lives in `config.yaml` under `conversation.system_prompt`. The default agent is **Aanya**. Identity enforcement is prepended in `app/services/conversation.py:create_context()`.
...
### Agent Personas
Six agents defined in `config.yaml` under `personas.agents`: **Aanya**, **Arjun**, **Priya**, **Serena**, **Rohan**, **Zara**. Each has a unique voice ID, personality, greeting lines, and domain scope. Any agent can call `transfer_to_agent()` mid-conversation — the LLM system prompt swaps silently, TTS voice switches instantly, and the new agent receives full conversation context.
...
8. **Live agent transfer** - In-session persona swap, no reconnect or reload required
